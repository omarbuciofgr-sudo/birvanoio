...
async function searchApolloDatabase(
  ...
  try {
    ...
    // Industry: numeric IDs only for organization_industry_tag_ids; otherwise use keyword search
    if (params.industry?.trim()) {
      const ind = params.industry.trim();
      if (/^\d+$/.test(ind)) {
        searchBody.organization_industry_tag_ids = [ind];
      } else {
        searchBody.q_organization_keyword_tags = [...((searchBody.q_organization_keyword_tags as string[]) || []), ind];
        searchBody.q_keywords = [searchBody.q_keywords as string, ind].filter(Boolean).join(' ').trim();
      }
    }
    if (params.keywords?.length) {
      const tags = params.keywords.map((k) => String(k).trim()).filter(Boolean);
      if (tags.length) {
        searchBody.q_organization_keyword_tags = [...((searchBody.q_organization_keyword_tags as string[]) || []), ...tags];
        const qk = [searchBody.q_keywords as string, ...tags].filter(Boolean).join(' ');
        searchBody.q_keywords = qk;
      }
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!params.industry?.trim() && !params.keywords?.length && (params.location?.city || params.location?.state)) {
      params.keywords = [params.location?.city, params.location?.state].filter(Boolean) as string[];
    }

    const hasIndustry = !!params.industry?.trim();
    const hasKw = !!(params.keywords && params.keywords.length > 0);
    const hasLoc = !!(params.location?.city || params.location?.state || params.location?.country);
    const hasTitles = !!(params.targetTitles && params.targetTitles.length > 0);
    if (!hasIndustry && !hasKw && !hasLoc && !hasTitles) {
      return new Response(
        JSON.stringify({
          success: false,
          error: 'Add at least one filter: industry, location, keywords, or job titles.',
        }),
