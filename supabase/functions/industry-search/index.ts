...
async function searchApollo(input: CompanySearchInput, apiKey: string): Promise<CompanyResult[] | null> {
  ...
  if (keywords) {
    // Merge keywords into keyword tags AND q_keywords (Apollo applies both; tags alone were ignoring extra phrases)
    ...
    if (keywordTags.length > 0) {
      ...
      const prevKw = typeof searchParams.q_keywords === 'string' ? searchParams.q_keywords.trim() : '';
      searchParams.q_keywords = [prevKw, ...keywordTags].filter(Boolean).join(' ');
    }
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let companies = Array.from(mergedMap.values());
    ...
    if (companies.length === 0) {
      return new Response(
        JSON.stringify({
          success: true, companies: [], total: 0, provider: usedProviders.join('+').toLowerCase() || 'none',
          pagination: { page, per_page: limit, total_entries: 0, total_pages: 0 },
        }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
