...
async function enrichWithClay(
  ...
  try {
    ...
    const response = await fetchWithRetry(`https://api.clay.com/v3/people/enrich?${params.toString()}`, {
      method: "GET",
      headers: {
    ...
    if (data.full_name || data.name) {
      result.full_name = data.full_name || data.name;
    ...
    if (data.job_title || data.title) {
      result.job_title = data.job_title || data.title;
      fields.push("job_title");
    }
    if (data.seniority) {
      result.seniority_level = data.seniority;
    ...
    if (data.company_annual_revenue) {
      result.annual_revenue =
        typeof data.company_annual_revenue === "string"
    ...
    console.error("Clay waterfall error:", error);
  }
  return { data: null, fields: [] };
}
...
async function enrichWithSnovio(
  ...
  try {
    ...
    if (data.success && data.data?.emails?.length) {
      ...
      if (!currentData.email && best.email) {
      ...
      if (!currentData.job_title && best.position) {
        result.job_title = best.position;
        fields.push("job_title");
      }
    ...
    console.error("Snov.io waterfall error:", error);
  }
  return { data: null, fields: [] };
}
...
async function enrichWithRocketReach(
  ...
  try {
    ...
    if (data.profiles?.length) {
      ...
      if (!phonesOnly) {
        if (!currentData.full_name && profile.name) {
          result.full_name = profile.name;
          fields.push("full_name");
        }
        if (!currentData.email && profile.current_work_email) {
          result.email = profile.current_work_email;
          fields.push("email");
        }
        if (!currentData.email && !result.email && profile.emails?.length) {
          result.email = profile.emails[0];
          fields.push("email");
        }
        if (!currentData.job_title && profile.current_title) {
          result.job_title = profile.current_title;
          fields.push("job_title");
        }
        if (!currentData.linkedin_url && profile.linkedin_url) {
          result.linkedin_url = profile.linkedin_url;
          fields.push("linkedin_url");
        }
        if (!currentData.company_name && profile.current_employer) {
          result.company_name = profile.current_employer;
          fields.push("company_name");
        }
      }
      if (!emailsOnly) {
    ...
    console.error("RocketReach waterfall error:", error);
  }
  return { data: null, fields: [] };
}
...
async function enrichWithRocketReachLookup(
  ...
  try {
    ...
    const response = await fetchWithRetry("https://api.rocketreach.co/v2/api/lookupProfile", {
      method: "POST",
    ...
    if (!response.ok) {
      console.log(
        `[strict_phone] rocketreach_lookup http=${response.status} snippet=${JSON.stringify(data).slice(0, 160)}`,
      );
      return { data: null, fields: [] };
    }
...
async function enrichWithLusha(
  ...
  try {
    ...
    const pns =
      (contact.phoneNumbers as unknown[]) ||
      (contact.phones as unknown[]) ||
      (raw.phoneNumbers as unknown[]) ||
      (raw.phones as unknown[]) ||
      [];
    if (!currentData.phone && Array.isArray(pns) && pns.length > 0) {
      const mobilePick = pns.find(
        (p: unknown) =>
          typeof p === "object" && p && String((p as { type?: string }).type || "").toLowerCase() === "mobile",
      );
      const directPick = pns.find(
        (p: unknown) =>
    ...
    console.error("Lusha waterfall error:", error);
  }
  return { data: null, fields: [] };
}
  ...
  try {
    ...
    if (currentData.linkedin_url) {
      ...
      if (response.ok) {
        ...
        if (!currentData.email && person.emails?.length) {
          result.email = person.emails[0];
          fields.push("email");
        }
        if (!currentData.phone && person.phones?.length) {
          result.phone = person.phones[0];
          fields.push("phone");
        }
        if (!currentData.full_name && person.name) {
          result.full_name = person.name;
          fields.push("full_name");
        }
        if (!currentData.job_title && person.title) {
    ...
    console.error("ContactOut waterfall error:", error);
  }
  return { data: null, fields: [] };
}
...
async function enrichWithClearbit(
  ...
  try {
    const response = await fetch(`https://company.clearbit.com/v2/companies/find?domain=${input.domain}`, {
      headers: { Authorization: `Bearer ${apiKey}` },
    });
...
Deno.serve(async (req) => {
  ...
  // Require authenticated user — protects paid enrichment API credits
  // (Apollo, Hunter, PDL, RocketReach, Lusha, Clearbit) from anonymous abuse.
  ...
  try {
    ...
    const companyName = String(input.company_name || body.organization_name || "").trim();
    ...
    if (shouldRunStrictB2bV1(body)) {
      ...
      if (wantEmail) {
        ...
        if (apolloApiKey && (apolloPersonId || linkedinForMatch)) {
          ...
          await runStep("apollo_person_match", apolloApiKey, !result.email, () =>
        ...
        await runStep("rocketreach_email", rocketreachApiKey, !result.email, () =>
      ...
      if (wantPhone) {
        ...
        await runStep("lusha", lushaApiKey, phoneStillMissingStrict(result), () =>
          enrichWithLusha(input, result, lushaApiKey!),
        );
        await runStep("rocketreach_lookup_phone", rocketreachApiKey, phoneStillMissingStrict(result), () =>
          enrichWithRocketReachLookup(input, result, rocketreachApiKey!),
        );
        await runStep("rocketreach_phone", rocketreachApiKey, phoneStillMissingStrict(result), () =>
          enrichWithRocketReach(input, result, rocketreachApiKey!, { phonesOnly: true }),
        );
        await runStep("pdl", pdlApiKey, phoneStillMissingStrict(result), () =>
      ...
      console.log(`[strict_b2b_v1] done. providers=${finalResult.providers_used.join(", ")} complete=${isComplete}`);
      return new Response(
        JSON.stringify({
          success: true,
          data: finalResult,
    ...
    if (availableProviders.length === 0) {
      return new Response(JSON.stringify({ success: false, error: "No enrichment providers configured." }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    await runStep("clay", clayApiKey, true, () => enrichWithClay(input, clayApiKey!));

    // Step 1: Apollo (always run — great for names, titles, phones)
    await runStep("apollo", apolloApiKey, true, () => enrichWithApollo(input, apolloApiKey!));
    ...
    await runStep("pdl", pdlApiKey, !hasCompleteData(result), () => enrichWithPDL(input, result, pdlApiKey!));
    ...
    await runStep(
      "clearbit",
      clearbitApiKey,
      !result.company_name || !result.industry || !result.employee_count || !result.annual_revenue,
    ...
    console.log(
      `Waterfall complete. Providers used: ${finalResult.providers_used.join(", ")}. Complete: ${isComplete}`,
    );
