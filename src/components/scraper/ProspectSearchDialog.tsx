...
export function BrivanoLens({ onSaveProspects }: BrivanoLensProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Search Tabs */}
      <Tabs value={searchMode} onValueChange={(v) => setSearchMode(v as any)} className="space-y-4">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="places" className="text-xs gap-1.5">
...
export function ProspectSearchDialog({ 
  ...
  onOpenChange,
  onSaveProspects,
}: { open: boolean; onOpenChange: (open: boolean) => void; onSaveProspects?: (prospects: ProspectResult[]) => void }) {
