...
export default function WebScraper() {
  ...
  useEffect(() => {
    ...
    if (!tab) return;
    if (tab === 'real-estate' || tab === 'rentcast') {
      setActiveTab('real-estate');
      return;
    }
    const allowed = ['ai-chat', 'planner', 'prospect-search', 'job-scraper', 'search'];
  ...
  const tabsInner = (
        <Tabs value={activeTab} onValueChange={setActiveTab} className={lensSearchTypeActive && activeTab === 'prospect-search' ? '' : 'space-y-4'}>
          ...
          {/* ── Real Estate Tab (RentCast FSBO/FRBO) ── */}
